...
def get_perspective_transform(box: np.ndarray, img: np.ndarray) -> np.ndarray:
    ...
    Matches PaddleOCR's get_rotate_crop_image: uses BORDER_REPLICATE to avoid
    black edge artifacts, INTER_CUBIC for quality, and auto-rotates tall crops.
    ...
    # Auto-rotate tall crops (vertical text) to horizontal
...
class OCRPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        Two modes:
        1. Directory mode: provide model_dir containing models/ subdirectory
        2. Explicit paths mode: provide det_model_path, rec_model_path, dict_path

        Args:
            model_dir: Path to directory containing models/ subdirectory
            det_model_path: Explicit path to detection ONNX model
            rec_model_path: Explicit path to recognition ONNX model
            dict_path: Explicit path to character dictionary
        ...
        if model_dir is not None:
            ...
            det_model_path = str(models_path / "det_model.onnx")
            rec_model_path = str(models_path / "rec_model.onnx")
            dict_path = str(models_path / "dictionary.txt")
        elif det_model_path and rec_model_path and dict_path:
            pass  # Use explicit paths
        else:
            raise ValueError(
                "Provide either model_dir or all three: "
                "det_model_path, rec_model_path, dict_path"
            )

        for label, path in [
            ("Detection model", det_model_path),
            ("Recognition model", rec_model_path),
        ...
        self.rec_postprocess = RecPostprocess(dict_path)

        if self.verbose:
            print(f"Loaded OCR pipeline")
    ...
    def predict(
        ...
        Returns:
            ...
            Returns empty list for unreadable images instead of raising.
        """
        # Handle single image
    ...
    def predict_with_details(
        ...
        """Run OCR on a single image with extra detection details.

        Like predict() but returns detection count alongside results.
        Each result dict includes an additional "det_score" key.

        Args:
            image: Image path or numpy array (BGR)

        Returns:
            Tuple of (results list, detection_count).
            Results include "text", "box", "confidence", "det_score".
            Returns ([], 0) for unreadable images.
        """
        return self._predict_single(image, include_det_score=True)

    def _predict_single(
        self, image: Union[str, np.ndarray], include_det_score: bool = False,
        ...
        if isinstance(image, str):
            ...
            if img is None:
                ...
                return ([], 0) if include_det_score else []
        else:
            img = image
        ...
        detection_count = len(boxes_and_scores)
        ...
        for box, det_score in boxes_and_scores:
            ...
            if text:  # Only add non-empty results
                ...
                if include_det_score:
                    result["det_score"] = float(det_score)
        ...
        if include_det_score:
            return results, detection_count
        return results
